    ...
    <>
      <div className="flex flex-wrap gap-1 mb-6" role="tablist" aria-label="Filter verdicts">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            <span className={`tnum text-[0.78rem] ${tab === t.id ? "opacity-80" : "text-ivory-3"}`}>
              {counts[t.id] ?? 0}
            </span>
          </button>
        ))}
  ...
  return (
    <article className="card overflow-hidden">
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-x-5 gap-y-2">
          <div>
            <p className="flex items-baseline gap-2.5">
              ...
              <span className="tnum text-[0.78rem] text-ivory-3">Case #{v.case_id}</span>
            </p>
            <p className="mt-1.5 serif text-[0.98rem] text-ivory-2">
              <Address value={v.plaintiff} />
              <span className="italic text-ivory-3 mx-1.5">v</span>
          ...
          <div className="text-right">
            <p className="tnum serif text-[1.6rem] leading-none" style={{ color: tone.color }}>
              {gen(v.award_wei)}
              <span className="text-[0.45em] text-ivory-3 ml-1">GEN</span>
            </p>
            <p className="tnum text-[0.79rem] text-ivory-3 mt-0.5">
      ...
      <div className="px-5 py-4 sm:px-6">
        {v.reasoning ? (
          <p className="serif text-[1rem] leading-[1.62] text-ivory-2 max-w-[74ch] line-clamp-4">
            {v.reasoning}
          </p>
        ) : null}
        <div className="mt-3.5 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[0.8rem] text-ivory-3">
